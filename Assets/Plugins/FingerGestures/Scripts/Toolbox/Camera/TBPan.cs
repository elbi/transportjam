...
public class TBPan : MonoBehaviour
{
	...
	public bool panInZ = false;
	public bool invertPan = false;
	private Vector2 screenSpaceMove;
	private bool ignoreDrag = false;

	public Vector2	ScreenSpaceMove { get { return screenSpaceMove; } }
	public bool		IgnoreDrag		{ get { return ignoreDrag;		} set { ignoreDrag = value; } }
    ...
    void Update()
    {
        if( dragGesture != null )
        {
            ...
            {
                screenSpaceMove = sensitivity * dragGesture.DeltaMove;
                Vector3 worldSpaceMove;
				if (panInZ)
					if (invertPan)
						worldSpaceMove = screenSpaceMove.x * cachedTransform.right + screenSpaceMove.y * cachedTransform.forward;
					else
						worldSpaceMove = screenSpaceMove.y * -cachedTransform.right + screenSpaceMove.x * cachedTransform.forward;
				else
					worldSpaceMove = screenSpaceMove.y * cachedTransform.right + screenSpaceMove.x * cachedTransform.up;
